...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-100 font-sans text-gray-900 overflow-x-auto">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col transition-transform duration-300 bg-white border-r border-gray-200 ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 ${sidebarOpen ? 'lg:w-56' : 'lg:w-16'}`}>
        <div className="h-12 flex items-center px-4 border-b border-gray-100 bg-gray-50">
          <div className="flex items-center gap-2 text-gray-900"><Hexagon size={20}/><span className="font-bold uppercase text-xs">AIR SALES</span></div>
        </div>
        <nav className="flex-1 py-4 px-2 space-y-1">
          {MODULES.filter(m => !m.adminOnly || currentUser.is_admin).map(m => (
            <button key={m.id} onClick={() => setActiveModuleId(m.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 transition-all ${activeModuleId === m.id ? 'bg-[#1a2130] text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
              <m.icon size={16} />{(sidebarOpen || mobileSidebarOpen) && <span className="text-[10px] uppercase font-semibold">{m.label}</span>}
            </button>
          ))}
        </nav>
        <div className="p-3 border-t border-gray-100">
            ...
            <button onClick={() => setCurrentUser(null)} className="w-full flex items-center justify-center gap-2 p-2 text-gray-400 hover:text-red-600 text-[10px] font-bold uppercase"><LogOut size={14} />Sair</button>
        </div>
      </aside>
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${sidebarOpen ? 'lg:ml-56' : 'lg:ml-16'}`}>
        ...
        <main className="p-2 space-y-2 w-full mx-auto">
          {activeModuleId === 'METAS' ? (
             <div className="grid grid-cols-12 gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className={`col-span-12 lg:col-span-4 bg-white border ${editingGoalId ? 'border-amber-500 shadow-amber-50' : 'border-gray-200 shadow-sm'} p-3 space-y-2 transition-all duration-300`}>
                   <div className="flex items-center justify-between border-b pb-2">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest">{editingGoalId ? 'Editando Meta' : 'Nova Meta de Vendas'}</h3>
                   ...
                   <div className="space-y-2">
                      <div>
                        ...
                        <select className="w-full px-2 py-1.5 bg-gray-50 border border-gray-200 text-[10px] mt-0.5 outline-none focus:border-gray-900" value={newGoal.rep_in_codigo} onChange={e => {
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <select className="w-full px-2 py-1.5 bg-gray-50 border border-gray-200 text-[10px] mt-0.5 outline-none" value={newGoal.ano} onChange={e => setNewGoal(p => ({...p, ano: Number(e.target.value)}))}>
                            {[2024, 2025, 2026, 2027].map(y => <option key={y} value={y}>{y}</option>)}
                          </select>
                        </div>
                        <div>
                          <label className="text-[9px] font-black uppercase text-gray-400">Mês</label>
                          <select className="w-full px-2 py-1.5 bg-gray-50 border border-gray-200 text-[10px] mt-0.5 outline-none" value={newGoal.mes} onChange={e => setNewGoal(p => ({...p, mes: Number(e.target.value)}))}>
                      ...
                      <div>
                        ...
                        <div className="relative mt-0.5">
                          ...
                          <input type="number" className="w-full pl-7 pr-2 py-1.5 bg-gray-50 border border-gray-200 text-[10px] outline-none focus:border-gray-900" value={newGoal.valor_meta} onChange={e => setNewGoal(p => ({...p, valor_meta: Number(e.target.value)}))} placeholder="0.00" />
                        </div>
                      </div>
                   </div>
                   <div className="space-y-1.5 pt-1">
                     <button onClick={handleSaveGoal} className={`w-full py-2.5 text-white text-[9px] font-bold uppercase tracking-widest shadow-lg transition-all active:scale-[0.98] ${editingGoalId ? 'bg-amber-600 hover:bg-amber-700' : 'bg-gray-900 hover:bg-black'}`}>
                        {editingGoalId ? 'Confirmar Alterações' : 'Registrar Meta'}
                     </button>
                     {editingGoalId && (
                       <button onClick={handleCancelEdit} className="w-full py-2 bg-gray-100 text-gray-500 text-[9px] font-bold uppercase tracking-widest hover:bg-gray-200 transition-all">
                          Cancelar Edição
                       </button>
                     )}
                   </div>
                    ...
                    <table className="w-full text-[10px]">
                      ...
                      <tbody className="divide-y">
                        {filteredGoals.length === 0 ? (
                          <tr><td colSpan={4} className="p-4 text-center text-[9px] font-bold text-gray-300 uppercase tracking-widest">Nenhum registro para este filtro</td></tr>
                        ) : (
                          filteredGoals.map(g => (
                            <tr key={g.id} className={`hover:bg-gray-50 transition-colors group ${editingGoalId === g.id ? 'bg-amber-50' : ''}`}>
                              <td className="px-4 py-2"><strong>{g.rep_nome}</strong><br/><span className="text-[8px] text-gray-400">COD: {g.rep_in_codigo}</span></td>
                              <td className="px-4 py-2 text-center font-bold text-gray-600">{MONTHS.find(m => m.id === g.mes)?.label} / {g.ano}</td>
                              <td className="px-4 py-2 text-right font-mono font-bold text-gray-900">{currencyFormat(g.valor_meta)}</td>
             ...
             <div className="grid grid-cols-12 gap-4 animate-in fade-in duration-300">
                    ...
                    <table className="w-full text-[10px]">
                        <thead className="bg-gray-50/50 border-b">
                            <tr className="text-[9px] font-black uppercase text-gray-400">
                                ...
                                <th className="px-4 py-2 text-center">Nível / Vínculo</th>
                        ...
                        <tbody className="divide-y">
                            {appUsers.map(u => (
                                <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-2 text-center">
                                        {u.is_admin ? (
                                            <span className="px-2 py-0.5 bg-gray-900 text-white text-[8px] font-black uppercase tracking-widest rounded-full flex items-center gap-1 w-fit mx-auto">
                                                <Shield size={8} /> Admin
                                            </span>
                                        ) : (
                                            <span className="px-2 py-0.5 bg-blue-50 text-blue-600 border border-blue-100 text-[8px] font-black uppercase tracking-widest rounded-full flex items-center gap-1 w-fit mx-auto">
                                                Rep: {u.rep_in_codigo || 'N/A'}
                                            </span>
                                        )}
                                    </td>
                                    <td className="px-4 py-2 text-right">
                                        <button onClick={async () => { if(confirm(`Remover acesso de ${u.name}?`)) { await deleteAppUser(u.id!); loadAppUsers(); } }} className="p-1.5 text-gray-300 hover:text-red-500 transition-colors">
                                            <Trash2 size={14}/>
                                        </button>
                                    </td>
                                </tr>
            ...
            <>
              {/* PAINEL DE FILTROS OTIMIZADO - GRID RESPONSIVO */}
                ...
                {filtersExpanded && (
                  <div className="p-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-2 items-end">
                    ...
                    <div className="space-y-0.5">
                      ...
                      >
                        <option value="">{currentUser.is_admin ? 'Todos' : 'Meu Cadastro'}</option>
                        {availableReps.map(r => <option key={r.code} value={r.code}>{r.name}</option>)}
                      </select>
                    </div>
                    <div className="space-y-0.5">
              ...
              <div className="bg-white border border-gray-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                <div className="p-2 border-b bg-gray-50 flex flex-wrap gap-2 items-center justify-between">
                   <div className="flex items-center gap-2"><TableIcon size={14}/><h3 className="text-[9px] font-bold uppercase tracking-widest">Visão Analítica</h3></div>
                      ...
                      {showColumnSelector && (
                        <div className="absolute right-0 mt-3 w-64 bg-white border border-gray-200 shadow-2xl z-40 animate-in fade-in zoom-in-95 duration-200 rounded-sm">
                          <div className="p-3 border-b flex justify-between items-center bg-gray-50">
                            <span className="text-[9px] font-black uppercase text-gray-900 tracking-widest">Layout</span>
                            <button onClick={() => setShowColumnSelector(false)}><X size={12} className="text-gray-400 hover:text-red-500"/></button>
                          </div>
                          <div className="py-2 max-h-[300px] overflow-y-auto custom-scrollbar">
                            {salesColumns.map(col => (
                              <button key={col.key} onClick={() => toggleColumnVisibility(col.key)} className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-50 transition-colors group text-left border-b border-gray-50 last:border-0">
                                <div className={`w-4 h-4 border rounded-sm flex items-center justify-center transition-all ${col.visible ? 'bg-gray-900 border-gray-900' : 'bg-white border-gray-300'}`}>
                                  {col.visible && <CheckCircle2 size={10} className="text-white"/>}
                                </div>
                                <span className={`text-[9px] font-bold uppercase tracking-widest flex-1 ${col.visible ? 'text-gray-900' : 'text-gray-300'}`}>{col.label}</span>
                              </button>
                            ))}
                          </div>
