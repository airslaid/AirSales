...
export default function App() {
  ...
  const [selectedSaleDetail, setSelectedSaleDetail] = useState<{summary: Sale, items: Sale[]} | null>(null);
  ...
  const getSharedLayoutKey = () => {
    const suffix = activeModuleId === 'ENTREGA' ? '_ent' : '';
    return `airsales_global_layout_v5_${currentUser?.email}${suffix}`;
  };
  ...
  const handleUpdateDelayReason = async (row: Sale, reason: string) => {
    const keys = { fil: Number(row.FIL_IN_CODIGO), ser: String(row.SER_ST_CODIGO), ped: Number(row.PED_IN_CODIGO) };
    setSalesData(prev => prev.map(item => {
        if (Number(item.FIL_IN_CODIGO) === keys.fil && String(item.SER_ST_CODIGO) === keys.ser && Number(item.PED_IN_CODIGO) === keys.ped) {
            return { ...item, MOTIVO_ATRASO: reason };
        }
        return item;
    }));
    try {
        await updateSaleDelayReason(keys, reason);
        showNotification("Motivo de atraso atualizado!", "success");
    } catch (error: any) {
        showNotification(`Erro ao atualizar motivo: ${error.message}`, "error");
    }
  };
  ...
  const generateColumns = (data: Sale[]) => {
    ...
    if (savedLayout) {
      try {
        ...
        // Se houver novas colunas obrigatórias no código que não estão no layout salvo, vamos injetá-las.
        const mandatoryKeys = ["DETAILS_ACTION", "ITP_RE_VALORTOTAL", "ITN_RE_QUANTIDADE"];
        const missingMandatory = mandatoryKeys.filter(mK => !parsedLayout.some(c => c.key === mK));
        
        if (missingMandatory.length > 0) {
            missingMandatory.forEach(key => {
                parsedLayout.push({
                    key,
                    label: key === 'DETAILS_ACTION' ? 'DETALHES' : key.replace(/_/g, ' ').replace('ITP RE', '').replace('ITP ST', '').trim(),
                    visible: true,
    ...
    const defaultOrder = [
      "PED_IN_CODIGO", "CLIENTE_NOME", "FILIAL_NOME", "PED_DT_EMISSAO", 
      "PED_ST_STATUS", "ITP_ST_DESCRICAO", "ITP_RE_QUANTIDADE", 
      "ITP_RE_VALORMERCADORIA", "ITP_RE_VALORTOTAL", "REPRESENTANTE_NOME", "DETAILS_ACTION"
    ];

    // Incluir colunas de entrega apenas no módulo de indicadores de entrega
    if (activeModuleId === 'ENTREGA') {
      defaultOrder.push("STATUS_ENTREGA", "DIAS_ATRASO", "MOTIVO_ATRASO");
    }

    const allPossibleKeys = ["SER_ST_CODE", "PED_IN_CODIGO", "CLI_IN_CODIGO", "CLIENTE_NOME", "FIL_IN_CODIGO", "FILIAL_NOME", "PED_DT_EMISSAO", "PED_CH_SITUACAO", "PED_ST_STATUS", "REP_IN_CODIGO", "REPRESENTANTE_NOME", "ITP_IN_SEQUENCIA", "ITP_ST_SITUACAO", "IT_ST_STATUS", "NF_NOT_IN_CODIGO", "NOT_DT_EMISSAO", "PRO_ST_ALTERNATIVO", "PRO_IN_CODIGO", "ITP_ST_DESCRICAO", "ITP_RE_QUANTIDADE", "ITP_RE_VALORUNITARIO", "ITP_RE_VALORMERCADORIA", "ITN_RE_VALORMERCADORIA", "ITN_RE_VALORTOTAL", "ITN_RE_QUANTIDADE", "ITP_RE_VALORTOTAL", "ITP_ST_PEDIDOCLIENTE", "IPE_DT_DATAENTREGA", "STATUS_ENTREGA", "DIAS_ATRASO", "MOTIVO_ATRASO", "TPD_IN_CODIGO", "CFOP_ST_DESCRICAO", "DETAILS_ACTION"];
    setSalesColumns(allPossibleKeys.map(key => ({ 
      key, 
      label: key === 'DETAILS_ACTION' ? 'DETALHES' : key.replace(/_/g, ' ').replace('ITP ST', '').replace('ITP RE', '').replace('PED RE', '').replace('PED ST', '').replace('IT ST', '').replace('IPE DT', '').replace('DT', 'DATA').trim(), 
  ...
  const processedData = useMemo(() => {
    ...
    // Filtros globais NÃO devem afetar o CRM. Ele possui seus próprios controles dentro da sua interface.
    if (activeModuleId !== 'CRM') {
  ...
  const metrics = useMemo(() => {
    ...
    // De-duplicação para métricas: agrupa por item real para não somar múltiplas vezes se houver várias notas
    const itemMap = new Map<string, Sale>();
    sourceData.forEach(d => {
      const baseSeq = (Number(d.FIL_IN_CODIGO) === 900) ? Number(d.ITP_IN_SEQUENCIA) : (Number(d.ITP_IN_SEQUENCIA) % 1000 || Number(d.ITP_IN_SEQUENCIA));
      const itemKey = `${d.FIL_IN_CODIGO}-${d.SER_ST_CODIGO}-${d.PED_IN_CODIGO}-${baseSeq}`;
      
      // Se já temos o item, verificamos se o dado atual tem NF (priorizamos a linha com NF para o faturado)
      if (!itemMap.has(itemKey)) {
        itemMap.set(itemKey, d);
      } else {
        const existing = itemMap.get(itemKey)!;
        const currentHasNF = (d['NOT_DT_EMISSAO'] && d['NOT_DT_EMISSAO'] !== '') || (d['NF_NOT_IN_CODIGO'] && Number(d['NF_NOT_IN_CODIGO']) > 0);
        if (currentHasNF) itemMap.set(itemKey, d);
      }
    });
    ...
    // O faturado deve somar TODOS os registros de notas fiscais, sem de-duplicação por item, 
    // pois um item pode ser faturado em várias notas (parciais).
    sourceData.forEach(d => {
      const vInvoice = parseBrNumber(d['ITN_RE_VALORTOTAL'] || 0);
      const s = String(d.PED_ST_STATUS || '').toLowerCase(); 
      const hasInvoice = (d['NOT_DT_EMISSAO'] && d['NOT_DT_EMISSAO'] !== '') || (d['NF_NOT_IN_CODIGO'] && Number(d['NF_NOT_IN_CODIGO']) > 0); 
  ...
  const performanceData = useMemo(() => {
      ...
      const itemMap = new Map<string, Sale>();
      relevantSales.forEach(d => {
        const baseSeq = Number(d.ITP_IN_SEQUENCIA) % 1000 || Number(d.ITP_IN_SEQUENCIA);
        const itemKey = `${d.FIL_IN_CODIGO}-${d.SER_ST_CODIGO}-${d.PED_IN_CODIGO}-${baseSeq}`;
        if (!itemMap.has(itemKey)) {
          itemMap.set(itemKey, d);
        } else {
          // Prioriza linha com NF se houver duplicata de item (clone)
          const currentHasNF = (d['NOT_DT_EMISSAO'] && d['NOT_DT_EMISSAO'] !== '') || (d['NF_NOT_IN_CODIGO'] && Number(d['NF_NOT_IN_CODIGO']) > 0);
          if (currentHasNF) itemMap.set(itemKey, d);
        }
      });
      ...
      } else if (perfSelectedReps.length > 0) {
  ...
  if (!currentUser) { 
    return ( 
      <div className="h-screen w-screen flex items-center justify-center p-4 relative overflow-hidden bg-transparent"> 
        <MeshGradientBackground />
        <div className="w-full max-w-md bg-white/70 backdrop-blur-3xl border border-white/20 shadow-[0_32px_128px_-16px_rgba(0,0,0,0.08)] rounded-none overflow-hidden relative z-10 animate-in fade-in zoom-in duration-1000"> 
          <div className="p-8 border-b border-gray-100/50 bg-white/50 flex items-center gap-6"> 
            <img src={logo} alt="Logo" className="h-12 w-auto object-contain select-none" />
            <div>
              <h1 className="text-xl font-black tracking-tighter text-gray-900 uppercase">AIR SALES</h1>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Analytics 4.0</p>
            </div> 
          </div> 
          <form onSubmit={handleLogin} className="p-10 space-y-6"> 
            {loginError && <div className="p-3 bg-red-50 border-l-4 border-red-500 text-[11px] font-bold text-red-600 rounded-none">{loginError}</div>} 
            <div className="space-y-4"> 
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-1">E-mail de Acesso</label>
                <input type="email" className="w-full px-4 py-3 bg-gray-50/50 border border-gray-100 rounded-none text-sm text-gray-900 focus:border-red-500/50 focus:bg-white outline-none transition-all placeholder:text-gray-300" placeholder="exemplo@empresa.com" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} /> 
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-1">Senha</label>
                <input type="password" className="w-full px-4 py-3 bg-gray-50/50 border border-gray-100 rounded-none text-sm text-gray-900 focus:border-red-500/50 focus:bg-white outline-none transition-all placeholder:text-gray-300" placeholder="••••••••" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} /> 
              </div>
            </div> 
            <button type="submit" className="w-full py-4 bg-gray-900 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-none hover:bg-black hover:shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.98]">Acessar Painel</button> 
          </form> 
        </div> 
      </div> 
    ); 
  ...
  return ( 
    <div className="flex h-screen w-screen bg-gray-100 font-sans text-gray-900 overflow-hidden print:h-auto print:overflow-visible print:bg-white"> 
      {/* Modais e Overlays */}
      <AIInsightsModal 
        isOpen={showAIModal}
      ...
      <aside className={`print:hidden flex flex-col border-r border-gray-200 bg-white transition-all duration-300 z-30 flex-shrink-0 ${mobileSidebarOpen ? 'fixed inset-y-0 left-0 shadow-2xl w-64 translate-x-0' : 'hidden lg:flex relative'} ${!mobileSidebarOpen && sidebarOpen ? 'w-56' : ''} ${!mobileSidebarOpen && !sidebarOpen ? 'w-16' : ''} `}> 
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full min-h-0 overflow-hidden relative print:block print:h-auto print:overflow-visible"> 
        <header className="print:hidden h-12 bg-white border-b border-gray-200 px-4 flex justify-between items-center shrink-0 z-10 shadow-sm"> 
        ...
        <main className="flex-1 flex flex-col min-h-0 overflow-hidden p-2 sm:p-4 w-full print:block print:overflow-visible print:p-0"> 
          ...
          {activeModuleId === 'OVERVIEW' ? (
            ...
            <div className="flex flex-col flex-1 min-h-0 gap-2 overflow-hidden"> 
                ...
                <div className="flex-1 overflow-hidden relative"> 
                  {activeModuleId === 'ENTREGA' && showDeliveryChart ? (
                    ...
                    <SalesTable 
                      ...
                      onShowDetails={(summary, items) => setSelectedSaleDetail({ summary, items })} 
                    /> 
                  )} 
                </div> 
              </div> 
              {selectedSaleDetail && (
                <SaleDetailModal 
                  summary={selectedSaleDetail.summary} 
                  items={selectedSaleDetail.items} 
