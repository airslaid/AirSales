...
interface PendingXmlItem {
  ...
  fileName: string;
  productCode: string;
  productDescription: string; // Adicionado para guardar o xProd do XML
  isExisting?: boolean;
}
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (activeModuleId !== 'PERFORMANCE' && activeModuleId !== 'COMISSAO' && activeModuleId !== 'PAGAMENTOS') { generateColumns(filtered); }
      ...
      reader.onload = (e) => {
        try {
          ...
          if (infNFe && ide && dest) {
             ...
             // Extração do Código (cProd) e Descrição (xProd) do Produto
             ...
             newItems.push({
               ...
               productDescription: productDescription // Salva a descrição extraída
             });
          }
        } catch (err) {} finally {
          processedCount++;
  ...
  const handleEditManual = (row: Sale) => {
     setPendingXmls([{
        ...
        productDescription: row.ITP_ST_DESCRICAO || "", // Na edição, a descrição atual
     ...
     try {
       const salesToSync: Sale[] = pendingXmls.map(item => {
          ...
          return {
             ...
             "PED_IN_CODIGO": parseInt(item.orderNumber), "ITP_IN_SEQUENCIA": 1, 
             ...
             "ITP_ST_DESCRICAO": item.productDescription || `IMPORTAÇÃO XML NFE ${item.nfeNumber}`, // USA A DESCRIÇÃO DO PRODUTO DO XML
  ...
  const commissionData = useMemo(() => {
    ...
    const isPaymentModule = activeModuleId === 'PAGAMENTOS'; const isCommissionModule = activeModuleId === 'COMISSAO';
    let filteredByMode = processedData.filter(item => { const status = String(item.PED_ST_STATUS || '').toUpperCase(); const hasInvoice = !!item.NOT_DT_EMISSAO; if (isPaymentModule) { if (commissionViewMode === 'FATURADO') return status.includes('FATURADO') || hasInvoice; return !status.includes('FATURADO') && !hasInvoice && !status.includes('CANCEL'); } if (commissionViewMode === 'FATURADO') { return status.includes('FATURADO') || hasInvoice; } return !status.includes('FATURADO') && !hasInvoice && !status.includes('CANCEL'); });
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative"> 
        ...
        <main className="flex-1 flex flex-col overflow-hidden p-2 sm:p-4 w-full"> 
          {activeModuleId === 'METAS' ? ( 
            ...
            </div> ) : activeModuleId === 'PERFORMANCE' ? ( 
            ...
            </div> ) : ( <div className="flex flex-col h-full gap-2 overflow-hidden"> <div className="bg-white border border-gray-200 shadow-sm overflow-hidden animate-in fade-in duration-300 shrink-0"> <div className="p-2 border-b bg-gray-50 flex items-center justify-between cursor-pointer" onClick={() => setFiltersExpanded(!filtersExpanded)}> <div className="flex items-center gap-2"><Filter size={12} className="text-gray-900" /><span className="text-[9px] font-bold uppercase tracking-widest">Painel de Inteligência</span></div> <ChevronDown size={12} className={`transition-transform duration-200 ${filtersExpanded ? 'rotate-180' : ''}`} /> </div> {filtersExpanded && ( <div className="p-2 flex flex-wrap items-end gap-2"> <div className="w-44 space-y-0.5"> <label className="text-[8px] font-black uppercase text-gray-400 tracking-tighter">Busca Global</label> <div className="relative"><Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" /><input type="text" placeholder="ID, Produto, Status..." className="w-full pl-7 pr-2 py-1 bg-gray-50 border border-gray-200 text-[10px] focus:border-gray-900 outline-none" value={filters.globalSearch} onChange={e => setFilters({...filters, globalSearch: e.target.value})} /></div> </div> <div className="flex-1 space-y-0.5"> <label className="text-[8px] font-black uppercase text-gray-400 tracking-tighter">Cliente</label> <input type="text" placeholder="Nome do Cliente..." className="w-full px-2 py-1 bg-gray-50 border border-gray-200 text-[10px] focus:border-gray-900 outline-none" value={filters.cliente || ''} onChange={e => setFilters({...filters, cliente: e.target.value})} /> </div> <div className="flex-1 space-y-0.5"><label className="text-[8px] font-black uppercase text-gray-400 tracking-tighter">Representante</label><select className="w-full px-1.5 py-1 bg-gray-50 border border-gray-200 text-[10px] focus:border-gray-900 outline-none disabled:bg-gray-100" value={filters.representante} onChange={e => setFilters({...filters, representante: e.target.value})} disabled={!currentUser?.is_admin}><option value="">{currentUser?.is_admin ? 'Todos' : 'Meu Cadastro'}</option>{availableReps.map(r => <option key={r.code} value={r.code}>{r.name}</option>)}</select></div> <div className="flex-1 space-y-0.5"><label className="text-[8px] font-black uppercase text-gray-400 tracking-tighter">Status</label><select className="w-full px-1.5 py-1 bg-gray-50 border border-gray-200 text-[10px] focus:border-gray-900 outline-none" value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})}><option value="">Todos</option>{availableStatuses.map(s => <option key={s} value={s}>{s}</option>)}</select></div> <div className="flex-1 space-y-0.5"><label className="text-[8px] font-black uppercase text-gray-400 tracking-tighter">Filial</label><select className="w-full px-1.5 py-1 bg-gray-50 border border-gray-200 text-[10px] focus:border-gray-900 outline-none" value={filters.filial} onChange={e => setFilters({...filters, filial: e.target.value})}><option value="">Todas</option>{availableFiliais.map(f => <option key={f} value={f}>{f}</option>)}</select></div> <div className="flex-1 min-w-[180px] space-y-0.5"> <label className="text-[8px] font-black uppercase text-gray-400 tracking-tighter"> {((activeModuleId === 'COMISSAO' || activeModuleId === 'PAGAMENTOS') && commissionViewMode === 'FATURADO') ? 'Data Faturamento (Nota)' : 'Período de Emissão (Pedido)'} </label> <div className="flex items-center gap-1"><input type="date" className="flex-1 px-1.5 py-1 bg-gray-50 border border-gray-200 text-[9px] outline-none focus:border-gray-900" value={filters.startDate} onChange={e => setFilters({...filters, startDate: e.target.value})} title="Data Inicial" /><span className="text-gray-300 text-[9px]">-</span><input type="date" className="flex-1 px-1.5 py-1 bg-gray-50 border border-gray-200 text-[9px] outline-none focus:border-gray-900" value={filters.endDate} onChange={e => setFilters({...filters, endDate: e.target.value})} title="Data Final" /></div> </div> <div className="w-20"><button onClick={clearFilters} className="w-full px-3 py-1 bg-gray-100 text-gray-500 text-[9px] font-bold uppercase tracking-widest hover:bg-gray-200 transition-all text-center border border-gray-200">Resetar</button></div> </div> )} </div> <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-2 shrink-0"> {activeModuleId === 'PAGAMENTOS' ? ( <> <StatCard title="Total a Pagar (Pendente)" value={currencyFormat(paymentMetrics.toPay)} icon={AlertCircle} color="text-red-500" /> <StatCard title="Total Pago (Baixado)" value={currencyFormat(paymentMetrics.paid)} icon={CheckCircle2} color="text-green-600" /> <StatCard title="Projeção (Carteira)" value={currencyFormat(paymentMetrics.projected)} icon={Wallet} color="text-blue-500" /> </> ) : activeModuleId === 'COMISSAO' ? ( <> <StatCard title={commissionViewMode === 'FATURADO' ? "Total Gerado" : "Previsão Gerada"} value={currencyFormat(commissionData.reduce((acc, curr) => acc + (curr.VALOR_COMISSAO || 0), 0))} icon={commissionViewMode === 'FATURADO' ? Banknote : Wallet} color={commissionViewMode === 'FATURADO' ? "text-green-600" : "text-amber-500"} /> <StatCard title="Total Faturado (Venda)" value={currencyFormat(metrics.faturado)} icon={TrendingUp} color="text-blue-600" /> </> ) : ( <StatCard title="Total Bruto (Itens)" value={currencyFormat(metrics.total)} icon={DollarSign} color="text-gray-900" /> )} {activeModuleId !== 'OV' && activeModuleId !== 'DV' && activeModuleId !== 'COMISSAO' && activeModuleId !== 'PAGAMENTOS' && ( <div className="bg-white p-3 border border-gray-200 shadow-sm hover:border-gray-900 transition-colors relative overflow-hidden group flex flex-col justify-between"> <div className="flex items-center justify-between mb-1"><p className="text-[9px] font-bold text-gray-400 uppercase tracking-[0.2em]">Meta</p><Target size={14} className="text-blue-500" /></div> <div className="flex items-end justify-between"><h3 className="text-lg font-bold text-gray-900 tracking-tighter tabular-nums">{currencyFormat(metrics.goal)}</h3>{metrics.goal > 0 && (<span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-sm ${metrics.achievement >= 100 ? 'bg-green-100 text-green-700' : metrics.achievement >= 70 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{metrics.achievement.toFixed(1)}%</span>)}</div> {metrics.goal > 0 && (<div className="w-full h-1 bg-gray-100 mt-2 rounded-full overflow-hidden"><div className={`h-full ${metrics.achievement >= 100 ? 'bg-green-500' : metrics.achievement >= 70 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`} style={{ width: `${Math.min(metrics.achievement, 100)}%` }}></div></div>)} </div> )} {activeModuleId !== 'COMISSAO' && activeModuleId !== 'PAGAMENTOS' && ( <> <StatCard title={activeModuleId === 'OV' ? "Em Aprovação" : "Faturado"} value={currencyFormat(activeModuleId === 'OV' ? metrics.emAprovacao : metrics.faturado)} icon={activeModuleId === 'OV' ? ShieldCheck : TrendingUp} color={activeModuleId === 'OV' ? "text-amber-600" : "text-green-600"} /> <StatCard title="Aberto" value={currencyFormat(activeModuleId === 'OV' ? metrics.emAberto : (metrics.total - metrics.faturado))} icon={Receipt} color="text-blue-600" /> </> )} {activeModuleId !== 'PAGAMENTOS' && ( <StatCard title="Pedidos Únicos" value={metrics.count.toString()} icon={Package} color="text-gray-400" /> )} </div> <div className="bg-white border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0 relative"> <div className="p-2 border-b bg-gray-50 flex flex-wrap gap-2 items-center justify-between shrink-0"> {activeModuleId === 'COMISSAO' || activeModuleId === 'PAGAMENTOS' ? ( <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full sm:w-auto"> <div className="flex items-center gap-1 overflow-x-auto pb-1 max-w-[50%] scrollbar-hide"> {COMMISSION_ROLES.map(role => ( <button key={role.id} onClick={() => setActiveCommissionRole(role.id)} className={`px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest whitespace-nowrap transition-all flex items-center gap-2 border ${activeCommissionRole === role.id ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-400 border-gray-200 hover:border-gray-400 hover:text-gray-600'}`} > {role.id === 'ASSISTENTE' ? <Calculator size={10} /> : role.id === 'VENDEDOR' ? <Briefcase size={10} /> : role.id === 'GERENTE' ? <TrendingUp size={10} /> : <HeartHandshake size={10} />} {role.label} </button> ))} </div> <div className="flex bg-gray-100 p-0.5 rounded-sm border border-gray-200"> <button onClick={() => setCommissionViewMode('FATURADO')} className={`px-3 py-1 text-[9px] font-bold uppercase tracking-widest rounded-sm transition-all flex items-center gap-1.5 ${commissionViewMode === 'FATURADO' ? 'bg-white shadow-sm text-green-700' : 'text-gray-400 hover:text-gray-600'}`} > <Banknote size={12}/> Real (Faturado) </button> <button onClick={() => setCommissionViewMode('ABERTO')} className={`px-3 py-1 text-[9px] font-bold uppercase tracking-widest rounded-sm transition-all flex items-center gap-1.5 ${commissionViewMode === 'ABERTO' ? 'bg-white shadow-sm text-amber-600' : 'text-gray-400 hover:text-gray-600'}`} > <Wallet size={12}/> Projeção (Carteira) </button> </div> </div> ) : ( <div className="flex items-center gap-2"><TableIcon size={14}/><h3 className="text-[9px] font-bold uppercase tracking-widest">Visão Analítica (Detalhamento por Item)</h3></div> )} <div className="flex items-center gap-2 ml-auto"> {(activeModuleId === 'COMISSAO' || activeModuleId === 'PAGAMENTOS') && ( <button onClick={() => setFilterOnlyManual(!filterOnlyManual)} className={`px-3 py-1.5 border text-[9px] font-bold uppercase tracking-widest flex items-center gap-2 transition-all shadow-sm ${filterOnlyManual ? 'bg-indigo-600 border-indigo-700 text-white' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`} title="Exibir apenas notas importadas manualmente" > <DatabaseIcon size={12}/> {filterOnlyManual ? 'Manual Ativado' : 'Apenas Manuais'} </button> )} {activeModuleId === 'COMISSAO' && ( <div className="relative"> <input type="file" id="xml-upload" multiple accept=".xml" className="hidden" onChange={handleXmlUpload} /> <label htmlFor="xml-upload" className="px-2 py-1.5 bg-blue-50 border border-blue-200 text-blue-700 hover:bg-blue-100 text-[9px] font-bold uppercase tracking-widest flex items-center gap-1 transition-all shadow-sm cursor-pointer"> <UploadCloud size={12}/> Importar XML </label> </div> )} <button onClick={handleExportExcel} className="px-2 py-1.5 bg-green-50 border border-green-200 text-green-700 hover:bg-green-100 text-[9px] font-bold uppercase tracking-widest flex items-center gap-1 transition-all shadow-sm" title="Exportar para Excel" > <FileSpreadsheet size={12}/> Excel </button> <button onClick={handleExportPDF} className="px-2 py-1.5 bg-red-50 border border-red-200 text-red-700 hover:bg-red-100 text-[9px] font-bold uppercase tracking-widest flex items-center gap-1 transition-all shadow-sm" title="Exportar para PDF" > <FileType size={12}/> PDF </button> <button onClick={() => setIsGroupedByOrder(!isGroupedByOrder)} className={`px-3 py-1.5 border text-[9px] font-bold uppercase tracking-widest flex items-center gap-2 transition-all shadow-sm ${isGroupedByOrder ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`} > <ListTree size={12}/> {isGroupedByOrder ? 'Agrupado por Pedido' : 'Lista Plana'} </button> <div className="relative" ref={columnSelectorRef}> <button onClick={() => setShowColumnSelector(!showColumnSelector)} className="px-3 py-1.5 bg-gray-900 text-white text-[9px] font-bold uppercase tracking-widest flex items-center gap-2 hover:bg-black transition-all shadow-sm"><Columns size={10}/>Colunas</button> {showColumnSelector && ( <div className="absolute right-0 mt-3 w-64 bg-white border border-gray-200 shadow-2xl z-40 animate-in fade-in zoom-in-95 duration-200 rounded-sm"> <div className="p-3 border-b flex justify-between items-center bg-gray-50"><span className="text-[9px] font-black uppercase text-gray-900 tracking-widest">Layout</span><button onClick={() => setShowColumnSelector(false)}><X size={12} className="text-gray-400 hover:text-red-500"/></button></div> <div className="py-2 max-h-[300px] overflow-y-auto custom-scrollbar">{(activeModuleId === 'COMISSAO' || activeModuleId === 'PAGAMENTOS' ? commissionColumns : salesColumns).map(col => (<button key={col.key} onClick={() => toggleColumnVisibility(col.key)} className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-50 transition-colors group text-left border-b border-gray-50 last:border-0"><div className={`w-4 h-4 border rounded-sm flex items-center justify-center transition-all ${col.visible ? 'bg-gray-900 border-gray-900' : 'bg-white border-gray-300'}`}>{col.visible && <CheckCircle2 size={10} className="text-white"/>}</div><span className={`text-[9px] font-bold uppercase tracking-widest flex-1 ${col.visible ? 'text-gray-900' : 'text-gray-300'}`}>{col.label}</span></button>))}</div> <div className="p-3 border-t bg-gray-50"><button onClick={saveGlobalVision} className={`w-full py-2.5 text-[9px] font-bold uppercase tracking-widest text-white transition-all shadow-md active:scale-[0.98] ${layoutSaved ? 'bg-green-600' : 'bg-gray-900 hover:bg-black'}`}>{layoutSaved ? 'Salvo!' : 'Salvar Padrão'}</button></div> </div> )} </div> </div> </div> <div className="flex-1 overflow-hidden relative"> <SalesTable data={activeModuleId === 'COMISSAO' || activeModuleId === 'PAGAMENTOS' ? commissionData : processedData} columns={activeModuleId === 'COMISSAO' || activeModuleId === 'PAGAMENTOS' ? commissionColumns : salesColumns} sortConfig={sortConfig} onSort={s => setSortConfig(p => p?.key === s ? {key:s, direction:p.direction==='asc'?'desc':'asc'} : {key:s, direction:'asc'})} onColumnReorder={handleColumnReorder} isLoading={loading || syncing} isGroupedByOrder={isGroupedByOrder} onTogglePayment={handleTogglePayment} onEditManual={handleEditManual} onDeleteManual={handleDeleteManual} /> </div> </div> )} </main> </div> </div> );
}
